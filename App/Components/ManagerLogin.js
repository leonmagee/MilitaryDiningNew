...
class ManagerLogin extends Component {
  ...
  loginManager() {
    if (this.state.managerData) {
      ...
      this.state.managerData.map(item => {
        if (
          item.manager_email == this.state.email &&
          item.manager_password == this.state.password
        ) {
          loginFailed = false;
  ...
  updateSpecial() {
    api
      .setMessHallMessage(this.state.messHallId, this.state.message)
      .then(response => {
  ...
  componentDidMount() {
    ...
    AsyncStorage.getItem('@LoginUserMessHallId')
      .then(id => {
        if (id) {
          AsyncStorage.getItem('@LoginUserMessHallName')
            .then(name => {
              this.setState({
                messHallId: id,
                messHallName: name,
      ...
      var pageContent = (
        <View style={styles.mainWrap}>
          <Text style={styles.headerText}>
            Set Message for Current Manager's Special
          </Text>

          <Text style={styles.messHallName}>
            Mess Hall: {this.state.messHallName}
          </Text>
      ...
      var pageContent = (
        <View style={styles.mainWrap}>
          ...
          <Animated.View
            style={[
              styles.badCredsWrap,
              { opacity: this.state.badCredentialsOpacity },
            ]}>
